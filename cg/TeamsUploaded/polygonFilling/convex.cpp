...
void points::scanline(points p[], int n)
{
	...
	for (int y = ymax; y >= ymin; y--)
	{
		...
		{
			glBegin(GL_POINTS);
			l.dda_line(interx[i], y, interx[i + 1], y);
...
void display()
{
	if (!complete) {
		glPointSize(1);
		glBegin(GL_POINTS);
		l.dda_line(p[i - 2].x, p[i - 2].y, p[i - 1].x, p[i - 1].y);
		glEnd();
		glFlush();
	}
	else {
		glPointSize(1);
		glBegin(GL_POINTS);
		l.dda_line(p[i - 1].x, p[i - 1].y, p[0].x, p[0].y);
		glEnd();
		glFlush();
	}
}
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("ScanLine_Polygon_Filling");
	init();
